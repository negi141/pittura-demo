...
//        $("dialog-title").text = "空に〇〇を描いてみた";
        playStart();

//        toast.show('空に人差し指で描いてみよう');
    }

//    var ti = 0;
    var myDrawStyle = "pen-1";//, playDrawStyle = "pen-u";
...
//        var keisu = 1 - org*0.00392156862; 
    ...
    var drawClear = function() {
        ...
        //ctxBump.fillStyle = "rgb(0, 0, 0)";
        //ctxBump.fillRect(0, 0, imgCanvasBump.width, imgCanvasBump.height);
        material_blur.alphaMap.needsUpdate = true;
        //material_blur.bumpMap.needsUpdate = true;
//material_blur.map.needsUpdate = true;
    ...
    var setScene = function() {
        ...
        textureCloud = setTextureCloud(workBgNo);
...
//material_blur.map.needsUpdate = true;
    ...
    window.drawedValidate = function(){
        for (var i = 0 ; i < TSIZE ; i++) {
            for (var j = 0 ; j < TSIZE ; j++) {
                if (heightMap[i][j] !== 255) return true;
            }
        }
        return false;
    };
    ...
    var setTexture = function(no){
        texture  = new THREE.ImageUtils.loadTexture(
            '/images/tex/sky/sky' + no + '.jpg');
        return texture;
    }
    var setTextureCloud = function(no){
        textureCloud  = new THREE.ImageUtils.loadTexture(
            '/images/tex/sky/cloud' + no + '.jpg');
        return textureCloud;
    }
